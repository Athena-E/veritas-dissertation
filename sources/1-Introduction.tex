...
\emph{Proof-carrying code} (PCC) \cite{Necula1997PCC} offers a way to move the trust boundary down to the compiled artifact without verifying the compiler itself: the producer of low-level code also ships evidence that the code obeys a safety policy, and the consumer checks this evidence before execution. One way to realise this idea is \emph{type-preserving compilation}, where types carrying information about memory safety properties are preserved from the source language down to assembly. The typed low-level artefact can be checked by a \emph{verifier}, the only trusted component in this model.
...
This dissertation presents \textbf{Veritas}, a toolchain that applies the ideas behind PCC to compile source programs for a small systems language to a version of a Dependently Typed Assembly Language (DTAL) \cite{Xi2001DTAL}. The central claim is not that the compiler is correct, but that source-level refinement guarantees are carried into untrusted low-level output and re-checked before execution. This reduces the TCB from the compiler pipeline to the standalone verifier and its dependencies.

The trust argument above gives two axes for comparing toolchains. Figure~\ref{fig:safety-sem-taxonomy} separates two guarantees that are often conflated: semantic preservation and executable-level memory safety.
