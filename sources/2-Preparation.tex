...
This chapter establishes the conceptual and technical background for Veritas. It introduces the type-theoretic foundations that motivate the project: type soundness, dependent and refinement types, bidirectional type checking, SMT-based proof discharge, and type-preserving compilation (Section~\ref{sec:background}). It then places Veritas in the context of related work (Section~\ref{sec:related-work}), particularly systems that trade off expressiveness, automation, and trust in different ways. Section~\ref{sec:design-decisions} covers design decisions for the system architecture, trust model, and languages as well as challenges and how they were addressed. Sections~\ref{sec:requirements-anal}--\ref{sec:starting-point} contain the requirements analysis, software engineering approach, and starting point for the project.

% The goal of the chapter is to give insight to existing ideas and make precise which of them Veritas does (or does not) adopt, and why those choices are appropriate for a project centred on independently verifiable systems code.
...
\noindent Together, these yield type soundness. Starting from a well-typed program, repeated evaluation can neither get stuck in an ill-defined intermediate state nor escape the guarantees expressed by its type. In practice, this is the formal basis for claims such as ``a well-typed array access cannot go out of bounds'' or ``a well-typed arithmetic expression is not applied to a boolean''.
...
In a dependently typed language, types may depend on terms. The canonical example is the type of vectors indexed by their length, $\mathrm{Vec}(A, n)$, where $n$ is a natural number. A function that accesses the $i$-th element can require in its type that $0 \le i < n$, statically ruling out out-of-bounds accesses. Full dependent type systems, such as those in Coq and Agda~\cite{Bove2009Agda}, can express arbitrary propositions as types (via the Curry--Howard correspondence), but this generality comes at a cost: type checking may require the programmer to construct explicit proof terms, and type inference is generally undecidable.
...
Having established the intended trust architecture and the theoretical tools and verification mechanisms available, my next step was to determine how they should be realised in the design of the language, compiler architecture, and trust model. This section describes the key design decisions made during the preparation phase.
...
The type system extends simple types with singleton types, refinement types, function contracts (with bounded quantifiers), and loop invariants. For efficiency, singleton types are represented as a distinct type variant so constant folding on singletons avoids invoking the SMT solver entirely.

When considering the example programs I wanted Veritas to express, particularly array-based algorithms such as binary search, I needed a way to state array-wide and range-based properties such as sortedness. Scalar refinement types alone cannot express such global invariants, so I extended the specification language with \emph{bounded quantifiers} in specification positions. A precondition such as $\mathbf{forall}\ i\ \mathbf{in}\ 0..n\ \{\ \mathit{arr}[i] \le \mathit{arr}[i + 1]\ \}$ expresses a property over all elements of an array, going beyond scalar refinements to reason about aggregate data structures.

\subsection{DTAL target language design}

The target language is a variant of Xi and Harper's Dependently Typed Assembly Language~\cite{Xi2001DTAL}, extended to support the Veritas refinement type system and targeting x86-64. I designed the core DTAL to resemble a RISC assembly language since simple, explicit instructions are easier to type, easier to verify, and map naturally from the compiler’s SSA-style intermediate representation. The DTAL was then augmented with type annotations on registers, constraint assertions embedded in the instruction stream, and declared type states at basic block entry points.
