...
\noindent The following excerpt shows the loop header of a DTAL copy function. The label signature binds index variables $m$, $n$, $i$ and declares register types in terms of them:
...
At the more expressive end of the spectrum are systems such as F*~\cite{Swamy2016Dependent}, which supports full dependent types, and RefinedC~\cite{Sammler2021RefinedC}, which applies refinement and ownership types to C code to enable the construction of machine-checked proofs in Coq. Veritas deliberately accepts a weaker foundation---SMT-backed checking rather than foundational proof---to keep proof obligations automatic once specifications are supplied.
...
Having established the theoretical tools and verification mechanisms available, I present how they are realised in the design of the language, compiler architecture, and trust model.
...
To allow the implementation to proceed incrementally while always maintaining a clear trust boundary, I structured the implementation to move the verification boundary progressively lower in the pipeline:
...
% TODO: cut down listing of features
The Veritas source language is a small imperative language with C-like syntax. It supports integers, booleans, fixed-size arrays, mutable and immutable variables, for-loops, while-loops, and functions. The type system extends simple types with singleton types, refinement types, function contracts (with bounded quantifiers), and loop invariants. For efficiency, I represented singleton types a distinct type variant so constant folding on singletons avoids invoking the SMT solver entirely.

A key expressiveness choice was bounded quantification. Scalar refinements can prove facts about individual values, but some algorithms that I wanted to express (e.g., binary search) need array-wide properties such as sortedness. A precondition such as $\mathbf{forall}\ i\ \mathbf{in}\ 0..n\ \{\ \mathit{arr}[i] \le \mathit{arr}[i + 1]\ \}$ expresses such properties while keeping verification close to SMT-friendly fragments.

\subsection{DTAL target language design}
% TODO: define V-DTAL
The target language is a variant of Xi and Harper's DTAL~\cite{Xi2001DTAL}, extended for Veritas' refinement type system and targeting x86-64. Hereafter `V-DTAL' refers to the Veritas-specific variant, and `DTAL' will refer to Xi and Harper's DTAL.

The core instruction set is deliberately RISC-like: simple instructions are easier to type, easier to verify, and map naturally from the compiler’s SSA-style intermediate representation. V-DTAL is then augmented with type annotations on registers, constraint assertions embedded in the instruction stream, and declared type states at basic block entry points.

I also defined V-DTAL with its own \textit{physical register model} (registers \texttt{r0}--\texttt{r12}, \texttt{sp}, \texttt{fp}, \texttt{lr}) that maps directly to x86-64 registers. This allows verification to occur \textit{after} register allocation: the register allocator is untrusted, and its output is verified before the final (trivial) encoding to x86-64.

\subsection{Machine code encoding and ELF generation}

Rather than emitting assembly text and invoking an external assembler, Veritas encodes x86-64 instructions directly as bytes. This keeps an external assembler out of the TCB. The trusted path below physically-allocated DTAL is intentionally small: a trivial one-to-one instruction mapping, a byte encoder, and an ELF generator.

The choice of x86-64 as the target architecture was pragmatic: the development machine runs x86-64 Linux, enabling direct execution and testing of compiled binaries without emulation. Targeting a different ISA would require replacing only the instruction mapping, byte encoder, and register definitions, while leaving the DTAL type system and verifier structure largely unchanged.

\subsection{Verification via SMT}

I designed the Veritas constraint system such that frontend type checking and low-level verification could be delegated to an SMT solver.
The safety obligations Veritas supports are naturally expressed as arithmetic and logical implications over program values and SMT-solving provides an automated way to discharge these obligations without requiring manual proofs for routine constraints. Z3~\cite{DeMoura2008Z3}, developed by Microsoft Research, is one of the most widely used SMT solvers and is the solver used in this project. I chose it for its maturity and reliability, having won numerous SMT-COMP competitions \cite{Barrett2013SMTCOMP}.
...
The requirements of the language and compiler architecture raised several interacting design challenges that required extending or departing from the approaches in the literature.
...
\paragraph{Verification after register allocation.} In an initial phase of the implementation, the system verified DTAL with virtual registers and then trusted the large x86-64 lowering module. This placed a large, error-prone component inside the TCB. Redesigning the pipeline to verify \textit{after} register allocation and reduce the backend TCB required defining a physical DTAL register model, splitting the monolithic lowering into an untrusted physical allocation pass and a trivial direct encoder, and extending the verifier to handle physical instructions (prologues, spills, division expansion).

\paragraph{Type preservation across SSA join points.} At phi nodes in the SSA intermediate representation, two singleton types from different branches must be joined. The naive approach---widening to the base type $\mathtt{int}$---loses all refinement information, making downstream bounds proofs impossible. The solution was to attach \textit{existential constraints} to phi nodes, encoding that the value satisfies a constraint without fixing it to a concrete value. This mechanism was not part of Xi and Harper's original DTAL design and required extending both the TIR and the verifier.
...
\begin{itemize}
  ...
  \item Static safety guarantees for the supported core language: type-correct operations, fixed-size array bounds, non-zero division, checked \texttt{i64} overflow, and structurally consistent DTAL control-flow and register states.
\end{itemize}
...
\begin{itemize}
  ...
  \item Evaluation evidence that the verifier rejects modified DTAL violating safety obligations, including strengthened preconditions, invalid assertions, bounds failures, arithmetic-safety failures, and entry-state mismatches.
\end{itemize}
% TODO: why x86?
...
\begin{itemize}
  ...
  \item Safety checks for a scoped reference fragment, including ownership transfer, immutable borrowing, mutable-borrow exclusivity, and typed scalar-reference access.
...
\begin{itemize}
  ...
  \item Verification of termination, temporal heap safety for general dynamic allocation, full functional correctness of arbitrary algorithms, or safety of unrestricted external interfaces.
\end{itemize}
