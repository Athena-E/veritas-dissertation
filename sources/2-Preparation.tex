...
The fundamental theorem that connects type systems to safety is \emph{type soundness}: a well-typed program cannot `go wrong' at runtime~\cite{Milner1978Theory}. Wright and Felleisen formalised this via the \emph{progress} and \emph{preservation} lemmas~\cite{Wright1994Syntactic}: progress states that a well-typed expression is either a value or can take an evaluation step, and preservation states that evaluation preserves the type. Together, these guarantee that a well-typed program never reaches an undefined state during execution.
...
Veritas targets this gap: its type system extends a simple base type system (integers, booleans, arrays) with refinement types and singleton types that encode value-dependent invariants.
...
Bidirectional type checking, introduced by Pierce and Turner~\cite{Pierce2000Local}, structures a type system into two mutually recursive judgements: type synthesis ($\Gamma \vdash e \Rightarrow \tau$), which infers the type of an expression bottom-up from its subexpressions, and type checking ($\Gamma \vdash e \Leftarrow \tau$), which verifies that an expression has a given type top-down. The two modes are connected by a subsumption rule; to check an expression against a type $\tau$, one may synthesise a type $\sigma$ and then verify $\sigma <: \tau$.
...
Veritas is an imperative language with C-like syntax, taking inspiration from Xi and Harper's Xanadu. It is designed to be minimal but expressive enough to demonstrate the verification techniques described in Sections~\ref{sec:dependent-types}--\ref{sec:smt-based-ver}. The language supports integers, booleans, fixed-size arrays, mutable and immutable variables, for-loops, while-loops, and functions. The type system extends simple types with singleton types, refinement types, function contracts (with bounded quantifiers), and loop invariants. The full syntax and typing rules are presented in Chapter~\ref{ch:impl}.
...
The evaluation aims to assess three dimensions: \textit{correctness} (does the system accept safe programs and reject unsafe ones?), \textit{overhead} (what is the cost of verification?), and \textit{expressiveness} (what can the type system express, and how large is the trusted computing base?).
