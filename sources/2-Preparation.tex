...
This chapter establishes the conceptual and technical background for Veritas. It introduces the relevant type-theoretic foundations (Section~\ref{sec:background}) and places Veritas in the context of related work (Section~\ref{sec:related-work}), leading into the design decisions behind Veritas' implementation (Section~\ref{sec:design-decisions}). We then analyse the requirements Veritas should fulfil to achieve the intended trust architecture, and a sufficient level of language expressivity and compiler practicality in a systems-oriented setting (Section~\ref{sec:requirements-anal}), before presenting the software engineering approach (Section~\ref{sec:software-eng-methodology}) and starting point for the project (Section~\ref{sec:starting-point}).
...
\begin{table}[H]
  ...
  \begin{tabular}{p{0.1\textwidth} p{0.1\textwidth} p{0.1\textwidth} p{0.6\textwidth}}
    ...
    \hline
    \texttt{chumsky}\footnote{\url{https://docs.rs/chumsky/latest/chumsky/}} & v1.0.0-alpha.8   & MIT              & Parser combinator library used for the lexer and parser; it provides the parsing infrastructure, but all grammar rules are original.     \\
    \hline
