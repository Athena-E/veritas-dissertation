...
A central architectural decision is that the compiler is \textit{untrusted}. Rather than proving the compiler correct, Veritas relies on a standalone verifier that re-checks the compiler's low-level output. Compiler bugs that corrupt the emitted type-state and safety policy evidence should therefore cause rejection rather than acceptance of safety-inconsistent low-level output. Figure~\ref{fig:trust-pipeline} shows the resulting boundary: type information flows through every intermediate representation, but trust begins only at the verifier and direct encoder.

The compiler is type-preserving in the engineering sense that it retains and translates safety-relevant type evidence through the pipeline. While this dissertation does not formalise compiler preservation, it provides empirical evidence towards the claim via regression and robustness tests; a machine-checked preservation proof is future work.
...
\begin{enumerate}
  \item \textbf{Virtual DTAL verification}: the verifier checks DTAL with virtual registers. The TCB includes the verifier, Z3, register allocation, instruction selection, and the x86-64 encoder, ELF generator, and runtime assumptions.
  \item \textbf{Physical DTAL verification}: the verifier checks DTAL \textit{after} register allocation. Register allocation and instruction selection are removed from the TCB. The TCB includes the verifier, Z3, a direct encoder, ELF generator, and runtime assumptions.
...
Rather than emitting assembly text and invoking an external assembler, Veritas encodes x86-64 instructions directly as bytes. This keeps an external assembler out of the TCB. The trusted path below physically-allocated DTAL is intentionally small but not trivial: most non-pseudo physical instructions have a direct encoding rule, while prologues, epilogues, runtime linkage, label resolution, and ELF generation remain trusted implementation work.
...
The SMT solver forms part of the TCB; it is important to minimise the risk introduced by this trust---using an obscure solver would weaken this argument. Z3~\cite{DeMoura2008Z3}, developed by Microsoft Research, is one of the most widely used SMT solvers and is the solver used in this project. I chose it for its maturity and reliability, having won numerous SMT-COMP competitions \cite{Barrett2013SMTCOMP}. Z3 also natively supports the theories featured in Veritas (Section~\ref{sec:smt-based-ver}).
