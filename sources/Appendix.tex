...
$$
  \frac{
    \begin{array}{c}
      ...
      \textsf{arith-safe}(\oplus,a,b,\Phi \wedge \varphi_1 \wedge \varphi_2) \\
      C_\oplus = \exists a,b.\; \varphi_1 \wedge \varphi_2 \wedge c = a \oplus b \quad
      \tau_\textsf{res} = \exists c.\; \textsf{int}(c) \;\mathbf{where}\; C_\oplus
    \end{array}
...
$$
  ...
  }{\Phi; \Gamma; C \vdash \textsf{cmp } r_1, r_2 \Rightarrow \Gamma; C[\textsf{last} \mapsto (\textsf{idx}(\tau_1), \textsf{idx}(\tau_2))]} \quad (\textsc{D-Cmp})
$$

$$
  \frac{
    \Gamma(r_1) = \tau_1
  }{\Phi; \Gamma; C \vdash \textsf{cmp } r_1, n \Rightarrow \Gamma; C[\textsf{last} \mapsto (\textsf{idx}(\tau_1), n)]} \quad (\textsc{D-CmpImm})
$$
...
$$
  \frac{
    C(\textsf{last}) = (e_1,e_2)
  }{\Phi; \Gamma; C \vdash \textsf{branch } \textit{cond}\; L \Rightarrow \Gamma; C \quad \text{with } \Phi' = \Phi \wedge \neg(e_1 \textit{ cond } e_2) \text{ (fall-through)}} \quad (\textsc{D-Branch})^\star
$$

\noindent The taken edge to $L$ carries $\Phi \wedge (e_1 \textit{ cond } e_2)$.
The comparison state $C$ is invalidated by instructions that clobber the machine flags, so a conditional branch can only consume a live comparison.
