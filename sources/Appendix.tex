...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{p{0.16\linewidth} p{0.20\linewidth} p{0.34\linewidth} p{0.22\linewidth}}
    ...
    & V-DTAL rules in Appendix~\ref{app:dtal-rules}; feature-suite verifier acceptance. \\
    \hline
    Optimisation
    & virtual V-DTAL $\rightarrow$ virtual V-DTAL
    ...
    \hline
    Physical allocation
    & virtual V-DTAL $\rightarrow$ physical V-DTAL
    ...
    & Post-register-allocation verifier; spill and physical-instruction rules; V-DTAL tampering corpus. \\
    \hline
    Independent verifier
    & physical V-DTAL $\rightarrow$ accept/reject
    ...
    & accepted physical V-DTAL $\rightarrow$ x86-64/ELF
...
The argument proceeds by induction over the pipeline stages in Table~\ref{tab:pass-preservation-invariants}, with the final step discharged by the physical V-DTAL verifier.
...
%     \hline
%     \textbf{Source construct} & \textbf{TIR representation} & \textbf{V-DTAL representation} \\
%     \hline
...
%     & \texttt{.assert}; checked before being added to the V-DTAL constraint context \\
%     Path assumption
...
%     \hline
%   \end{tabular}
%   \caption{Correspondence between representative source constructs, TIR forms, and verifier-facing V-DTAL forms.}
...
\begin{table}[h]
  ...
  \begin{tabular}{p{0.33\textwidth} c c p{0.4\textwidth}}
    ...
    Tuple / product heap objects               & Yes                 & No               & Xi--Harper has \texttt{prod(...)} and \texttt{newtuple}; Veritas does not currently expose general tuple allocation in V-DTAL. \\
    Sum / tag reasoning                        & Yes                 & No               & Xi--Harper extends DTAL for sums and tag-check elimination; Veritas does not currently have this V-DTAL feature.               \\
    ...
    Physical post-regalloc verification target & No                  & Yes              & Veritas verifies physically allocated V-DTAL after register allocation.                                                        \\
    ...
    \hline
  \end{tabular}
  \caption{Feature comparison between Xi and Harper's DTAL and V-DTAL.}
