...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{p{0.16\linewidth} p{0.20\linewidth} p{0.34\linewidth} p{0.22\linewidth}}
    ...
    Lowering
    & TAST $\rightarrow$ TIR
    & Replace source variables by SSA registers while translating branch facts, contracts, loop invariants, bounds obligations, and ownership events into register-indexed constraints.
...
Lowering is syntax-directed. The induction hypothesis is that each already-lowered subexpression has a virtual register and a type/constraint description corresponding to the source judgement. Variable lookup is translated by the source-to-register environment; mutation becomes SSA reassignment; branches attach translated guard constraints to successor edges; and loop-carried values are represented by phi nodes. Existential constraints on phi nodes preserve facts such as induction-variable bounds without requiring a concrete singleton value. Thus TIR preserves the safety-relevant source evidence in a register-indexed form.
