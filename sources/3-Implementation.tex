...
This chapter implements the Chapter~\ref{ch:prep} trust model as an executable compiler-verifier pipeline. It presents the source language, typed intermediate representation, DTAL target, type-preserving compiler, independent verifier, direct x86-64 encoder, compact runtime, bare-metal target, and ownership extension as one system.
...
|-- src/
|   |-- common/                   # Shared AST, typed AST, and types
|   |-- frontend/                 # Lexer, parser, and type checker
|   |-- middle/
|   |   |-- tir/                  # Typed SSA intermediate representation
|   |   `-- lower/                # TAST-to-TIR lowering
|   |-- dtal/                     # DTAL syntax, constraints, and parser
|   |-- backend/
|   |   |-- codegen/              # TIR-to-DTAL code generation
|   |   |-- optimise/             # Typed DTAL optimisations
|   |   |-- regalloc/             # Liveness analysis and allocation
|   |   |-- physalloc/            # Physical allocation and spill insertion
|   |   |-- emit/                 # DTAL text emission
|   |   |-- x86_64/               # Target instruction forms and byte encoder
|   |   |-- direct_encode.rs      # Physical-DTAL to x86-64 encoding
|   |   |-- elf/                  # ELF generation and bare-metal output
|   |   `-- runtime.rs            # Hand-written runtime intrinsics
|   |-- verifier/                 # Independent DTAL verifier
|   |-- pipeline.rs               # Pipeline orchestration
|   `-- main.rs                   # Command-line interface
|-- eval/                         # Benchmarks, suites, and plotting scripts
`-- docs/                         # Dissertation and supporting material
\end{verbatim}
  \caption{Veritas project directory structure.}
  \label{fig:repo}
\end{figure}
...
Figure~\ref{fig:pipeline} summarises the compilation pipeline. Each named representation is shown in sequence, with arrows indicating transformations. Safety evidence lowers from source refinements into verifier-facing DTAL type states and constraints; the final verifier consumes the physical-register form produced after backend concretisation. After physical-DTAL validation, final x86-64 encoding no longer needs the annotations.
...
This section presents Veritas syntax and formal typing rules. They define the source-level contract the pipeline must preserve: any program accepted by the type checker satisfies the properties encoded in its types.

\subsection{Syntax}

The abstract syntax of Veritas is defined by Figure~\ref{fig:veritas-syntax}. The grammar uses these metavariables:

\begin{itemize}
  \item $n$: integer literals
  \item $x$: variables
  \item $f$: function names
  \item $N$: integer expressions that must reduce to a statically known constant.
\end{itemize}

If a block has a trailing expression, its value is the block's result; otherwise, its result is unit. $\textsf{aop}$ and $\textsf{uaop}$ range over binary and unary arithmetic operators; $\textsf{cop}$ over comparison operators; and $\textsf{lop}$ and $\textsf{ulop}$ over binary and unary logical operators. Grey text denotes an optional clause; $(\cdot)^*$ denotes zero or more repetitions.
...
The master type $\tau_m$ is set at declaration and never changes; the current type $\tau_c$ may be refined through control flow but must remain a subtype of $\tau_m$. When a mutable variable is declared as \texttt{let mut x:T = e}, $T$ is recorded as its master type $\tau_m$. Its \textit{current type} $\tau_c$ may narrow through assignments and control flow (e.g., after \texttt{if x > 0}, to $\{v\!:\!\mathtt{int} \mid v > 0\}$), but every assignment \texttt{x = e} must satisfy $\tau_e <: \tau_m$, where $\tau_e$ is the right-hand side's synthesised type. At control-flow joins, branch current types are merged by join, but the result must remain a subtype of the master type. This prevents refinement invalidation if \texttt{x} is reassigned. Section~\ref{sec:typechecker} describes the implementation.
...
\begin{itemize}
    ...
    \item Different integer singletons: $\textsf{int}(N_1) \sqcup \textsf{int}(N_2) = \{v \mid v = N_1 \vee v = N_2\}$ where $N_1 \ne N_2$.
...
\noindent When $\textsf{join}$ is used in expression rules (e.g., \textsc{T-If-Expr}), the same type-joining rules apply to branch result types. The implementation keeps this precision for integer singletons, so a branch assigning either $0$ or $1$ can still verify a later length-two array access. Boolean singleton joins are currently widened to \textsf{bool}, because Veritas' refinement predicates are integer-focused; more aggressive disjunctive joining is also limited by SMT formula growth.
...
\begin{itemize}
    \item \textbf{Synthesis:} $\phi; \Delta; \Gamma \vdash e \Rightarrow (\phi'; \Delta'; \tau)$ --- synthesise type $\tau$ for expression $e$, optionally updating the context.
...
The subtyping relation $\phi \vdash \tau_1 <: \tau_2$ is defined by structural rules (Appendix~\ref{app:source-rules}) and two SMT-backed rules. Mutable arrays are invariant in the checked fragment; covariant element reasoning is used only for read-only facts. In both solver-backed rules, propositions in $\phi$ are asserted as assumptions, the goal is negated, and the solver is checked for unsatisfiability:
...
For expressions used in proof obligations, synthesis returns both a type and an SMT-level index term:
\[
  \phi;\Delta;\Gamma \vdash e \Rightarrow (\phi';\Delta';\tau;\iota)
\]
The term $\iota$ denotes the expression's value in generated constraints. For a singleton it is the singleton index; for a refinement or existential type it is the symbolic term introduced during synthesis. Boolean expressions expose a guard proposition $\mathsf{guard}(e)$, for example $\mathsf{guard}(e_1 < e_2) = \iota_1 < \iota_2$. Conditional and loop rules use this guard, so branch facts such as \texttt{lo <= hi} enter the path context.

Arithmetic operations use a $\textsf{join-op}$ function that constant-folds singleton operands (e.g., $\textsf{join-op}(+, \textsf{int}(1), \textsf{int}(2)) = \textsf{int}(3)$) and otherwise synthesises a refinement type via SMT refinement synthesis (e.g., $\{v\!:\!\textsf{int} \mid v = e_L + e_R\}$):
...
\noindent $\textsf{arith-safe}$ requires non-zero divisors for division and modulo and includes the signed-64-bit range obligations checked by the current implementation. Arithmetic outside those emitted side conditions is not part of the scoped guarantee.
...
Standard rules for blocks, let bindings, array assignment, if-statements, and while loops appear in Appendix~\ref{app:source-rules}. Key rules are described below.
...
An important feature is \textit{refinement lifting}: an array typed as $[\{v\!:\!\textsf{int} \mid v \ge 0\}; N]$ automatically satisfies $\forall i \in [0, N).\; \mathit{arr}[i] \ge 0$. The checker extracts this quantified proposition from the array's element type and adds it to the context while the array is in scope.

\section{The DTAL target language}\label{sec:dtal-target-lang}

Veritas targets a first-order variant of Xi and Harper's DTAL design~\cite{Xi2001DTAL}. Xi and Harper formally established DTAL soundness, proving that well-typed programs cannot violate memory safety or type-system constraints. Veritas adapts that idea to an executable compiler pipeline whose low-level artifact carries enough evidence for a standalone checker to reconstruct the relevant safety argument.
...
This section describes V-DTAL's concrete instruction set and typing judgements.

\subsection{Instruction set}\label{sec:dtal-instr-set}

The DTAL instruction set resembles a standard RISC assembly language augmented with type annotations and constraint assertions. DTAL appears in two forms: a virtual-register form emitted by code generation, and a physical-register form produced after register allocation. Both carry verifier-facing type information, and the final verifier checks the physical form. The core syntax is summarised in Figure~\ref{fig:dtal-syntax}.
...
Beyond executable instructions, textual DTAL contains directives describing function metadata, block entry states, and verifier annotations. Figure~\ref{fig:dtal-directives} shows where these directives appear in a typical DTAL function.
...
Function-level directives define the verifier-facing signature and contract. Block-level directives declare the type state required on block entry, and edge checking proves that predecessor states imply these entry states. Constraint directives split into assumptions, part of the declared block state, and assertions, checked at their program point before entering the context. Physical directives such as \texttt{.prologue} and \texttt{.epilogue} appear only after register allocation.

\subsection{DTAL type system}

The DTAL type system largely mirrors the source-level type system: unit, integer and boolean base types, machine-integer types, singleton integers $\textsf{int}(N)$, refinement types, arrays, and master types. The main difference is that these types classify \emph{registers} rather than source variables, so type indices are low-level expressions over register contents. DTAL index expressions may mention constants, registers, arithmetic, and $\textsf{select}(\mathit{arr}, i)$ for array element access (of array $\mathit{arr}$ atindex $i$).

The main addition is \textit{existential types} for results known only symbolically at assembly level. When arithmetic on non-singleton operands cannot preserve an exact singleton type, the result is represented as an existential such as $\exists x.\; \textsf{int}(x) \;\mathbf{where}\; \varphi(x)$, where $\varphi$ is a predicate over integer witnesses. This retains enough logical structure for later proof obligations without requiring every intermediate value to have a concrete singleton type.

The DTAL constraint language preserves the source-level specification fragment: comparisons, logical connectives, and bounded quantifiers. It is represented separately as a first-order language over DTAL index expressions rather than directly as source expressions, and conversion from source expressions is partial. These constraints are threaded through the verifier's type state and refined by comparisons, branches, assertions, and function contracts.

\subsection{DTAL typing judgements}

DTAL type checking operates over a \textit{type state}. The rules below show its core as $(\Phi, \Gamma)$, where $\Gamma$ maps registers to current types and $\Phi$ is the constraint set at a program point. The implementation also records auxiliary verifier state such as comparison/flag state $\mathcal{F}$, the typed stack, array versions, spill slots, and proven assertions. The core instruction-typing judgement is $\Phi; \Gamma \vdash \textsf{instr} \Rightarrow \Gamma'$, meaning that executing $\textsf{instr}$ under constraints $\Phi$ and register state $\Gamma$ produces register state $\Gamma'$. Rules reading or updating machine flags use $\Phi; \Gamma; \mathcal{F} \vdash \textsf{instr} \Rightarrow \Gamma'; \mathcal{F}'$.
...
A memory load is well-typed only if $\Phi$ can prove the index within bounds. The result type is the array's element type:
$$
...
\noindent where $e_i$ is the index expression extracted from the type of $r_i$. The verifier derives the result type from the array base rather than trusting the annotation, and records a constraint equating $r_d$ with the corresponding versioned array selection. Stores are symmetric, with one addition: the verifier maintains \textit{array versions}, so each store creates a fresh logical array name. It then adds explicit write and frame axioms over versioned $\textsf{select}$ expressions, avoiding conflation between values written at different program points.

\paragraph{Conditional Branching}
A comparison instruction updates the verifier's comparison state. We write $\mathcal{F}(\textsf{last})$ for the live symbolic operand pair recorded by the most recent comparison; this field is undefined after any instruction that clobbers machine flags. The auxiliary function $\textsf{idx}(\tau)$ extracts the index expression associated with an integer-like DTAL type, such as a singleton, refined, or existential integer type:
$$
\frac{
...
\noindent The rule for \texttt{setcc} consumes the same comparison state but only assigns the destination register a boolean type. The verifier uses branch refinements, not boolean-register refinements, for path-sensitive bounds proofs:
$$
\frac{
...
\noindent Each outgoing state is then coerced to the successor block's declared entry state. This verifies bounds checks: a comparison followed by branch-if-greater-or-equal exits the loop, and the fall-through path continues with the index known in bounds. Instructions clobbering machine flags invalidate $\mathcal{F}(\textsf{last})$, so branches and \texttt{setcc} can only consume live comparisons.
...
The lexer and parser are built using the \texttt{chumsky} parser combinator library. The frontend uses a restricted expression parser inside array sizes, singleton types, and refinement predicates, and a full expression parser for ordinary source expressions. This avoids a circular dependency between type parsing and statement/block parsing, keeps specification positions to pure expressions, and prevents block-level constructs from being accepted in type positions only to be rejected later.
...
The type checker is structured around the typing context $(\phi, \Gamma, \Delta, \Sigma_F)$. Contexts are immutable values, not shared global state: each judgement takes a context and returns an updated one. The \texttt{im} crate makes this practical with persistent (structurally shared) data structures: cloning a context at a control-flow branch is $O(\log n)$ rather than $O(n)$. This supports functional-style context threading in bidirectional type checking, where each \textbf{if-else} branch receives its own context copy.
...
The implementation provides an explicit synthesis combinator, \texttt{synth\_expr}, which takes a typing context and expression and returns either a type error or a typed expression with its synthesised type. Integer literals produce singleton types, variable lookups return the type from $\Gamma$ or $\Delta$, and arithmetic operations use the \texttt{join\_op} helper from Section~\ref{sec:expr-typing-rules}. The checking judgement is implemented by synthesising a type, then using SMT-backed subtyping to verify conformance to the expected type.

Statement checking (\texttt{check\_stmts}) updates the typing context according to each statement's effect: \texttt{let} bindings extend $\Gamma$ or $\Delta$, assignments update the current type in $\Delta$, and loops require invariants to hold on entry and be preserved by the body, as in Section~\ref{sec:stmt-typing-rules}.
...
When both operands of an arithmetic operation are singletons, the type checker performs constant folding at the type level: $\mathtt{int}(a) \oplus \mathtt{int}(b) = \mathtt{int}(a \oplus b)$, thereby avoiding an SMT query entirely.

When one or both operands are not singletons, the checker performs \textit{refinement synthesis}: it constructs a fresh refinement type $\{v\!:\!\mathtt{int} \mid v = e_L \oplus e_R\}$ capturing the exact arithmetic relationship between result and operands. The bound variable $v$ gets a fresh name (e.g., \texttt{\_synth\_0}) to avoid capture. This lets downstream code reason precisely about the result---for instance, to verify that \texttt{lo + (hi - lo) / 2} lies within bounds, given $\mathtt{lo} \ge 0$ and $\mathtt{hi} < n$.
...
The SMT oracle wraps Z3 behind one entry point: $\texttt{is\_provable}(\phi, P)$, returning whether proposition $P$ is valid under assumptions $\phi$. Internally, it creates a fresh Z3 context, translates each proposition in $\phi$ to a Z3 expression, asserts them, asserts $\lnot P$, and checks unsatisfiability.

This translation maps Veritas specification expressions and propositions into Z3 terms and formulae. Integer expressions map to the Z3 integer sort; boolean expressions to the Z3 boolean sort. Arrays use Z3's built-in array sort (\texttt{Array(Int, Int)}), where \texttt{select} corresponds to indexing. Bounded quantifiers such as $\mathbf{forall}\ x\ \mathbf{in}\ e_1..e_2\ \{P\}$ become $\forall x.\; (e_1 \le x \wedge x < e_2) \Rightarrow P$ using Z3's native quantifier support. While this moves outside the decidable quantifier-free fragment, Z3's E-matching heuristics handle the bounded quantifiers that arise in practice.

An important feature is \textit{refinement lifting}: when an array has a refined element type (e.g., $[\{v\!:\!\mathtt{int} \mid v \ge 0\}; n]$), the oracle extracts a universally quantified proposition ($\forall i \in [0, n).\; \mathit{arr}[i] \ge 0$) and adds it to the assumptions. This lets element-level refinements support quantified reasoning without programmer restatement.
...
Type errors are \texttt{TypeError} variants carrying source spans, expected and actual types, and context. A separate reporting module formats diagnostics using \texttt{ariadne}, highlighting the relevant source location and explaining the failed proof obligation, including assumed propositions and the unproved goal.
...
A key extension beyond standard SSA is \textit{existential constraints on phi nodes}. Each phi node may carry an optional existential constraint of the form $\exists v.\; \mathtt{int}(v) \;\mathbf{where}\; \varphi(v)$, encoding that the phi's value satisfies $\varphi$ without fixing a concrete value. For loop induction variables, this captures bounds: $\exists v.\; \mathtt{int}(v) \;\mathbf{where}\; (v \ge \mathit{start} \wedge v < \mathit{end})$. For mutable variables with refined types, it preserves the refinement predicate across iterations. This avoids the information loss of simply widening phi nodes to the base type.
...
A TIR function stores its parameters, pre/postconditions, and basic-block map. Each block contains entry phi nodes, ordinary instructions, a terminator, and an entry register state. This makes code generation largely structural while retaining the source-level type information needed for later verification.
...
The grammar abstracts over the Rust data structures used in the implementation. The implementation follows the same decomposition directly: function and block structs contain typed parameters, phi nodes, instruction lists, terminators, and entry states, while enum variants represent the individual instruction and terminator forms in Figure~\ref{fig:tir-syntax}.

\subsection{Lowering from TAST to TIR}

The lowering pass translates the typed AST to TIR by assigning source bindings to virtual registers and constructing a control-flow graph. Translation largely follows convention: expressions become three-address instructions, mutable assignments create fresh SSA values, and structured control flow becomes basic blocks and terminators. Veritas differs by preserving proof-relevant frontend information.

Source-level constraints are rewritten from source variables to virtual registers. The lowering context maintains the current source-to-register map, so variable constraints are emitted over the registers currently holding those values:
\begin{itemize}
  \item branch conditions become explicit true and false edge constraints;
  \item loop invariants are rewritten over the current loop-carried registers;
  \item array reads and writes carry the bounds constraints already proved by the type checker.
\end{itemize}
...
%                   minimum height=0.8cm, text width=0.28\textwidth},
%       arrow/.style={-Latex, thick}
%     ]
%     \node[box] (src) {TAST constraint\\over source names\\\texttt{lo <= hi}};
%     \node[box, right=of src] (map) {lowering context\\\texttt{lo -> v11}\\\texttt{hi -> v10}};
%     \node[box, right=of map] (tir) {TIR constraint\\over registers\\\texttt{v11 <= v10}};
%
%     \draw[arrow] (src) -- (map);
%     \draw[arrow] (map) -- (tir);
%   \end{tikzpicture}
...
%   \caption{Lowering rewrites source-level proof obligations into verifier-facing TIR constraints.}
%   \label{fig:tast-to-tir-pipeline}
% \end{figure}

\begin{figure}[H]
  \newcommand{\flowkw}[1]{\textcolor{blue!70!black}{#1}}
  \newcommand{\flowlo}[1]{\textcolor{teal!70!black}{#1}}
  \newcommand{\flowhi}[1]{\textcolor{orange!85!black}{#1}}
  \newcommand{\flowmid}[1]{\textcolor{red!70!black}{#1}}
  \newcommand{\flowarr}[1]{\textcolor{violet!75!black}{#1}}
  \newcommand{\flowob}[1]{\textcolor{green!45!black}{#1}}
  \newcommand{\flowval}[1]{\textcolor{magenta!70!black}{#1}}
  \newcommand{\flowblk}[1]{\textcolor{gray!75!black}{#1}}
  \centering
  \begin{minipage}{0.47\textwidth}
    \footnotesize\ttfamily
    \flowkw{while} \flowlo{lo} \flowob{<=} \flowhi{hi} \flowkw{invariant} \flowlo{lo} \flowob{>= 0} \flowkw{\textbraceleft}\\
    \null\hspace*{1em}\flowkw{let} \flowmid{mid}: int = \flowlo{lo} + (\flowhi{hi} - \flowlo{lo}) / 2;\\
    \null\hspace*{1em}\flowkw{let} \flowval{val}: int = \flowarr{arr}[\flowmid{mid}];\\
    \null\hspace*{1em}\flowblk{...}\\
    \flowkw{\textbraceright}
  \end{minipage}
  \hspace{0.03\textwidth}
  \begin{minipage}{0.47\textwidth}
    \footnotesize\ttfamily
    \flowkw{branch} \flowob{v\_cmp} \flowblk{bb\_body} \flowblk{bb\_exit}\\
    \null\hspace*{1em}\flowkw{where} \flowlo{v11} \flowob{<=} \flowhi{v10}, !(\flowlo{v11} \flowob{<=} \flowhi{v10})\\
    \flowkw{assert} \flowlo{v11} \flowob{>= 0}\\
    \flowval{v20} := \flowkw{load} \flowarr{v\_arr}[\flowmid{v19}] : int\\
    \null\hspace*{1em}\flowkw{where} \flowob{0 <=} \flowmid{v19} \flowob{\&\&} \flowmid{v19} \flowob{< 10}
  \end{minipage}
  \captionsetup{aboveskip=4pt, belowskip=2pt}
  \caption{Source-level control-flow and memory-safety obligations after lowering to TIR; matching colours show how source facts are rewritten over virtual registers.}
  \label{fig:tast-to-tir-snippets}
\end{figure}

Joins also preserve refinement information. Phi nodes for loop-carried or branch-merged values may carry existential constraints, for example recording that a loop induction variable lies in $[\mathit{start},\mathit{end})$ without fixing its concrete value. This gives DTAL generation enough information to produce verifier-checkable block-entry type states.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[smallblock] (entry)  at (0,0) {entry\\\texttt{i = 0}};
    \node[phiblock] (header) at (5.6,0)
      {loop header\\
       \texttt{v\_i = phi(0, v\_next)}\\
       $\exists v.\,\mathsf{int}(v)\ \mathbf{where}\ 0 \le v < 10$};
    \node[block]    (body)   at (9.8,-3.6)
      {body\\
       \texttt{load arr[v\_i]}\\
       requires $0 \le v_i < 10$};
    \node[block]    (back)   at (5.6,-3.6)
      {back edge\\
       \texttt{v\_next = v\_i + 1}};
    \node[smallblock] (exit)   at (13,0) {exit};

    \draw[flow] (entry) -- (header);
    \draw[flow] (header.east) -| node[pos=0.28, right, font=\scriptsize] {$v_i < 10$} (body.north);
    \draw[flow] (body.west) -- (back.east);
    \draw[flow] (back.north) -- node[right, font=\scriptsize] {back edge} (header.south);
    \draw[flow] (header) -- node[above, font=\scriptsize] {$v_i \ge 10$} (exit);

  \end{tikzpicture}

  \captionsetup{aboveskip=4pt, belowskip=2pt}
  \caption{CFG showing how existential phi constraints preserve loop-range facts needed for a later array-bounds proof.}
  \label{fig:tir-existential-phi}
\end{figure}
...
Optimisation runs on virtual-register DTAL before physical allocation. The pipeline performs constant folding, local peephole rewrites, copy propagation, dead-code elimination, loop-invariant code motion, and a Veritas-specific load-operation fusion pass. Crucially, these transformations operate over typed assembly, not an untyped IR: explicit constants, bounds facts, and verifier-facing effects remain present, giving optimisation passes facts that ordinary low-level code has lost. Rewritten instructions must retain verifier-facing type annotations, and semantically or proof-relevant instructions---stores, calls, branches, stack operations, ownership events, and constraint assertions---are treated as effects rather than removable bookkeeping.

Two implementation details are specific to the later backend. First, constant folding may introduce immediate-form DTAL instructions such as \texttt{AddImm} and \texttt{CmpImm}, which have verifier rules and direct encoder cases. Second, load-operation fusion creates a \texttt{LoadOp} form to exploit x86-64 memory operands, but only when the original load has one arithmetic use. Copy propagation is similarly conservative around physical-register materialisation moves, since those copies encode ABI-oriented structure needed by physical allocation.

\subsection{Register allocation}

Register allocation uses standard liveness analysis followed by graph colouring. The default allocator is Chaitin--Briggs style over the 11 allocatable general-purpose x86-64 registers, excluding \texttt{rsp}, \texttt{rbp}, \texttt{rax}, \texttt{rdx}, and \texttt{r11}. Registers live across calls are constrained to callee-saved registers (\texttt{rbx}, \texttt{r12}--\texttt{r15}), reflecting the calling convention.

\subsection{Physical allocation}\label{sec:physalloc}

The register allocator maps virtual to physical registers; the \textit{physical allocation pass} applies this mapping to produce physical-register DTAL. This is the central transformation in the default pipeline, and it is \textit{untrusted}: the DTAL verifier checks its output before code generation. I refer to this final check as \textit{physical-DTAL validation}: the safety argument is reconstructed on the post-register-allocation artifact, after backend transformations expose concrete registers, stack slots, calling-convention moves, and target-specific instruction expansions. This is the strongest trust claim, because the verifier checks the form immediately before byte encoding rather than an idealised compiler IR.
...
Beyond register renaming, physical allocation performs several x86-specific transformations:
...
since \texttt{idiv} requires its operands in the \texttt{rdx:rax} pair.
  \item \textbf{Caller-saved register management}: at each call site, caller-saved registers live across the call are spilled before argument setup and restored after return using explicit \texttt{SpillStore} and \texttt{SpillLoad} instructions, with the verifier checking spill-slot type consistency.
  \item \textbf{Parameter move cycle resolution}: when function parameters must be shuffled between registers (e.g., parameter~0 in \texttt{rdi} to \texttt{rsi}, while parameter~1 in \texttt{rsi} to \texttt{rcx}), the pass detects cycles and breaks them using \texttt{r11} as scratch.
  \item \textbf{Constraint remapping}: all type constraints are rewritten from virtual register names (\texttt{v3}) to physical register names (\texttt{r0}), enabling physical-program verification.
\end{itemize}

\subsection{Direct encoding}\label{sec:direct-encoding}

The \textit{direct encoder} translates physically-allocated DTAL to x86-64 machine code. Because physical allocation has already performed all x86-specific expansions (division, calling convention, prologues), the encoder is a deliberately narrow direct mapping: each DTAL instruction maps to one x86-64 instruction, or a short fixed prologue/epilogue sequence. The module is 309 lines.

This directness is deliberate and architecturally significant. Initially, I implemented a monolithic x86-64 lowering pass performing register allocation, instruction selection, and encoding in one trusted step. Splitting this into an untrusted physical allocation pass, verified by the DTAL verifier, and a compact encoder moves most register-allocation and instruction-selection logic outside the trusted path. The encoder remains trusted because it is the final unchecked step from validated physical DTAL to bytes; its compactness supports auditing that step.

\subsection{Binary encoding and ELF generation}

The direct encoder emits x86-64 machine code from physical DTAL, using a two-pass scheme to resolve label offsets. The byte stream is then wrapped in a minimal ELF executable with entry point \texttt{main}; the Linux target uses ELF64, while the bare-metal target uses the Multiboot-compatible layout below.

\section{Independent verifier}\label{sec:verifier}

A key contribution of the type-preserving architecture is that physically-allocated DTAL can be verified independently of the compiler. The verifier re-checks the DTAL program from scratch, using only embedded assembly type annotations. It follows Xi and Harper's derivation-based approach~\cite{Xi2001DTAL}: types are \textit{re-derived} from instruction semantics, not trusted from compiler annotations. This means that spills, reloads, register shuffles, prologues, epilogues, and division expansion are validated in the form immediately preceding direct machine-code encoding.
...
\begin{enumerate}
  ...
  \item Symbolically executes each instruction, \textit{re-deriving} the result type from operand types and instruction semantics.
  ...
  \item At block exits (jumps, branches, returns), performs \textit{state coercion}: checking that the computed exit state is compatible with each successor's declared entry state.
\end{enumerate}

The re-derivation in step 2 is central to the untrusted-compiler model. For example, \texttt{mov rd, 5} always derives $\texttt{rd} : \mathtt{int}(5)$ from the immediate, regardless of compiler annotation. Similarly, \texttt{add rd, r1, r2} derives $\texttt{rd} : \mathtt{int}(\texttt{r1} + \texttt{r2})$ using symbolic index algebra. Compiler annotations are only cross-checks; if the re-derived type conflicts with the annotation, the verifier reports an error. Trust attaches to the checker derivation, not compiler provenance.

\subsection{State coercion at control-flow edges}

State coercion checks compatibility across block boundaries. When block A jumps to block B, A's exit state must be a subtype of B's declared entry state. This reduces to two checks:

\begin{itemize}
  \item \textbf{Register type subtyping}: for each register in B's entry state, A's exit type must be a subtype. The relation is constraint-aware: $\mathtt{int}(n) <: \mathtt{int}$ always holds, and $\mathtt{int}(n) <: \exists v.\; \mathtt{int}(v) \;\mathbf{where}\; \varphi(v)$ holds if $\varphi(n)$ is provable from A's constraints.
  \item \textbf{Constraint entailment}: every constraint in B's entry state must be provable from A's exit constraints.
\end{itemize}

For conditional branches, the verifier first refines each edge's exit state. For example, \texttt{cmp r1, r2} followed by \texttt{jl target} contributes $\texttt{r1} < \texttt{r2}$ on the taken edge and $\texttt{r1} \ge \texttt{r2}$ on the fall-through edge before coercion against successor states.

\subsection{Array bounds verification}

Array loads and stores are the verifier's most critical instructions. For each \texttt{load rd, [base + offset]}:
...
\begin{enumerate}
  ...
  \item It proves this obligation using syntactic checks (e.g., is the constraint already in context?) or the SMT oracle.
...
\end{tcolorbox}
The array version increments on each store, ensuring that the SMT solver can distinguish array states before and after mutations.
...
This enables modular verification: each function is checked in isolation via contracts rather than inlining.

\subsection{SMT oracle}

The verifier checks emitted DTAL from scratch using its own SMT queries; no proof state passes from source checker to verifier. Translation of DTAL constraints into SMT queries follows the compiler oracle's proof-by-refutation approach (Section~\ref{sec:smt-oracle}).

For performance, constraint provability uses two tiers. A fast syntactic check handles common cases (constraint already in context, simple entailment patterns) in $O(n)$ where $n$ is the context size. The SMT solver is invoked only when this is inconclusive. This matters because the verifier issues one query per instruction rather than one per subtyping check.
...
%
% This is a deliberate architectural choice. Register allocation is one of the most error-prone compiler passes (it involves liveness analysis, graph colouring, spill heuristics, and calling-convention details), and verifying its output rather than trusting it substantially reduces the TCB, evident in the implementation of the direct encoder (Section~\ref{sec:direct-encoding}).

\section{Runtime}\label{sec:runtime}

The compiler links a compact runtime intrinsic set into every executable. These are hand-assembled x86-64 machine code totalling approximately 210 bytes, prepended to the user's compiled code in the ELF output.
...
The first three intrinsics use Linux syscalls and are Linux-target only. The port I/O intrinsics use hardware instructions directly and are available on both Linux and bare-metal targets. The type checker enforces this distinction: a bare-metal program calling \texttt{print\_int} is rejected at compile time rather than failing at runtime.

All intrinsics follow the System~V AMD64 calling convention (first argument in \texttt{rdi}, return value in \texttt{rax}), so compiled Veritas code calls them without marshalling. The hand-assembled runtime is part of the TCB, so the design keeps it compact, inspectable, and tested.

\section{Bare-metal target}\label{sec:unikernel}

The Veritas CLI's \texttt{--target-bare-metal} flag produces a Multiboot-compliant ELF32 that can boot directly on x86-64 hardware or QEMU\footnote{QEMU is an open-source machine emulator and virtualiser: \url{https://www.qemu.org/}.} without an operating system. This shows the same checked pipeline producing OS-independent executable artifacts. The host kernel is removed from the normal execution path and replaced by a compact trusted bootstrap and runtime. The bootstrap, runtime intrinsics, hardware model, and QEMU behaviour remain outside the DTAL verifier's guarantee.
...
The bootstrap is hand-assembled and part of the TCB; at $\sim$200 bytes it remains small enough for line-by-line audit.

\subsection{DTAL-checked serial driver}

To demonstrate DTAL-checked bare-metal I/O, the project includes a serial driver written entirely in Veritas. It programs the UART 16550 controller, the standard PC serial port at I/O base \texttt{0x3F8}, using \texttt{port\_in} and \texttt{port\_out}:
...
This demonstrates the project's long-term direction: a bare-metal Veritas target in which application code and device-facing drivers are checked as DTAL before encoding, while a compact trusted bootstrap and runtime replace the host operating-system kernel. Figure~\ref{fig:unikernel-arch} illustrates the architecture of a bare-metal Veritas binary.
...
Aliasing is the next pressure point for a systems-language verifier: mutation through one name can invalidate facts assumed through another, and unrestricted references obscure mutable-object ownership. Veritas addresses this with a Rust-inspired ownership and borrowing discipline: the source checker rejects obvious aliasing errors, while TIR and DTAL preserve ownership events explicitly so the final low-level verifier can check that the backend has not introduced an illegal move, duplicated owner, or conflicting borrow.
...
The supported fragment is lexical and first-order: borrows are tied to source bindings, block exit ends borrows introduced in that block, and references are not stored in aggregates or returned from functions. This keeps the model compatible with the Veritas type checker while supporting feature-suite systems patterns: shared array reads, mutable array updates, scalar dereference, and reference parameters.

\begin{figure}[H]
\begin{center}
\begin{cminted}{rust}
fn ownership_forms() -> int {
    let mut x: int = 0;
    {
        let rx: &int = &x; // shared reference type and shared borrow
        let y: int = *rx;  // scalar dereference through a read-only reference
    }                      // block exit ends the shared borrow of x
    {
        let mx: &mut int = &mut x; // unique mutable reference to mutable x
        *mx = 9;                   // assignment through *mx requires &mut int
    }                              // block exit ends the mutable borrow
    let arr: [int; 4] = [7; 4];
    let rarr: &[int; 4] = &arr;  // shared array reference
    rarr[0]                      // array-reference indexing proves 0 <= 0 < 4
}
\end{cminted}
\end{center}
\caption{Source-level ownership and borrowing forms in Veritas.}
\label{fig:source-ownership-forms}
\end{figure}

% \subsection{Frontend implementation}

Borrowing is implemented as an extension of the ordinary bidirectional type checker. In addition to the contexts $(\phi, \Gamma, \Delta \Sigma_F)$, the frontend context records ownership metadata:

\begin{itemize}
  \item \textbf{Moved bindings}: bindings whose ownership has been transferred and cannot be used again until reinitialised.
  \item \textbf{Active borrows}: for each owner, records live shared-borrow counts and whether a mutable borrow is live.
  \item \textbf{Borrow bindings}: maps reference bindings back to the owner they borrow from and the borrow kind.
  \item \textbf{Borrow scopes}: stack of lexical scopes used to release borrows at block exit.
\end{itemize}

Creating \texttt{\&x} increments the shared-borrow count for \texttt{x}; creating \texttt{\&mut x} requires no live shared or mutable borrow. Assignments, shadowing, moves, and function calls consult the same metadata, so the checker rejects mutating a shared-borrowed owner, moving a borrowed owner, creating a shared borrow while a mutable borrow is live, and creating two simultaneous mutable borrows. These checks are integrated into \texttt{check\_stmt} and \texttt{synth\_expr}, so the typed AST records both expression type and borrow/move behaviour needed by lowering.

% \subsection{TIR representation}

Once the frontend accepts a program, ownership behaviour becomes explicit in TIR. Later compiler stages need not reconstruct ownership events from source syntax: the verifier-facing pipeline receives first-class operations for moves, drops, borrow creation, and borrow termination.

\begin{figure}[H]
{\small
\[
\renewcommand{\arraystretch}{1.09}
\begin{array}{@{}r@{\ }c@{\ }l@{\hspace{1.2em}}p{0.28\textwidth}@{}}
  I_\mathsf{own} & ::= &
  \parbox[t]{0.58\textwidth}{
    $\rho_d := \textsf{move\_owned}\; \rho_s : \tau \,|\, \textsf{drop\_owned}\; \rho_s : \tau \,|\, {}$\\
    $\rho_d := \textsf{borrow\_shared}^{\ell?}\; \rho_s : \&\tau \,|\, {}$\\
    $\rho_d := \textsf{borrow\_mut}^{\ell?}\; \rho_s : \&\textsf{mut }\tau \,|\, {}$\\
    $\textsf{borrow\_end}^{\ell?}\; \rho_s : \&\tau \text{ or } \&\textsf{mut }\tau$
  }
  & \textbf{(Ownership instructions)} \\[4pt]
  a & ::= &
  \textsf{plain} \mid \textsf{owned} \mid \textsf{borrow} \mid \textsf{borrow\_mut}
  & \textbf{(Argument kinds)} \\[2pt]
  I & ::= &
  \cdots \mid I_\mathsf{own} \mid \optclause{\rho :=}\; \textsf{call}\; f(\overrightarrow{a\,\rho}) : \tau
  & \textbf{(Extended instructions)}
\end{array}
\]
}
\caption{Ownership-aware extension to the TIR syntax.}
\label{fig:tir-ownership-forms}
\end{figure}

Scalar references require one additional implementation device. Arrays already have a TIR and DTAL memory representation, so borrowing an array can target it directly. A scalar value, however, must be made addressable before borrowing. The lowerer uses a hidden one-element cell for scalar references: it allocates a size-1 cell, stores the scalar value, borrows the cell, and lowers dereference to an ordinary load or store through that cell. This lets scalar references reuse array load/store and bounds-checking machinery rather than introducing a separate pointer memory model.

% \subsection{DTAL representation}

The ownership-aware TIR forms lower to explicit DTAL instructions. The extension also adds shared and mutable reference types, written $\&\tau$ and $\&\textsf{mut } \tau$, which classify registers holding borrow capabilities. These instructions do not perform ordinary arithmetic computation; they update verifier ownership state and expose aliasing behaviour in the low-level artifact.
...
The key property is that the backend cannot silently erase the source-level ownership discipline. If it emits a second mutable borrow, moves an owner while a borrow is live, or fails to end a borrow on one control-flow path, the final DTAL program is inconsistent with the verifier's ownership state and is rejected.

% \subsection{Verifier support}

The independent verifier extends its ordinary register-and-constraint state with ownership metadata. It tracks registers holding owned values, assigns object identities, records shared or mutable borrows of each object, and records consumed registers. The instruction checker then enforces this capability discipline:

\begin{itemize}
    \item \textbf{Move availability:} \texttt{move\_owned} requires the source register to hold a live owned object.
    \item \textbf{Borrow exclusion:} \texttt{borrow\_mut} is rejected if the object has any live shared or mutable borrow.
    \item \textbf{Shared borrow compatibility:} \texttt{alias\_borrow} is allowed for shared borrows but rejected while a mutable borrow is live.
    \item \textbf{Borrowed-owner protection:} \texttt{move\_owned} and \texttt{drop\_owned} are rejected while the object is borrowed.
    \item \textbf{Borrow termination:} \texttt{borrow\_end} requires the register to hold a live borrow and releases it from the object state.
    \item \textbf{Control-flow joins:} join states must agree on live ownership and borrow information; one-sided borrow endings are rejected.
\end{itemize}
...
%   \item \textbf{The TCB is minimal.} The only components that must be correct are the verifier itself and the Z3 SMT solver. The entire compiler frontend --- lexer, parser, type checker, lowering, code generation, and optimisation --- is outside the TCB.
% \end{itemize}
