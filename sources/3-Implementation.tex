...
The two modes of the bidirectional system are implemented as separate functions. The function \texttt{synth\_expr} takes a typing context and an expression as input and returns either a type error or, on success, returns an updated context together with the synthesised type. Integer literals produce singleton types ($\mathtt{int}(n)$), variable lookups return the type from $\Gamma$ or $\Delta$, and arithmetic operations use the \texttt{join\_op} function described below. \texttt{check\_expr} takes a typing context, an expression, and an expected type as input and returns a type error or, on success, an updated context; unlike synthesis, it does not produce a new type, but verifies that the expression conforms to the expected one via the SMT-backed subtyping check.
