...
\begin{array}{@{}r@{\ }c@{\ }l@{\hspace{1.2em}}p{0.25\textwidth}@{}}
  ...
  \parbox[t]{0.66\textwidth}{
    $c \,|\, x \,|\, e \textsf{ aop } e \,|\, e \textsf{ cop } e \,|\, e \textsf{ lop } e \,|\, \texttt{!}\; e \,|\, \texttt{-}\; e \,|\, f(\overrightarrow{e}) \,|\, {}$\\
    $x[e_i] \,|\, \textsf{if } e_\textsf{cond}\; \{B\}\; \textsf{else } \{B'\} \,|\, [e_\textsf{val}; e_\textsf{size}]$
  }
  & \textbf{(Expressions)} \\[4pt]
  P & ::= &
  \parbox[t]{0.66\textwidth}{
    $e \textsf{ cop } e \,|\, P \textsf{ lop } P \,|\, \texttt{!}\; P \,|\, {}$\\
  ...
  D & ::= &
  \parbox[t]{0.66\textwidth}{
    $\textsf{fn } f(x_1:T_1, \ldots)\; (\rightarrow T_\textsf{ret})^?\; (\textsf{requires } P)^?\;$\\
    $(\textsf{ensures } P)^?\; \{B\}$
  }
  ...
  T & ::= &
  \parbox[t]{0.66\textwidth}{
    $\textsf{unit} \,|\, \textsf{int} \,|\, \textsf{bool} \,|\, [T; N] \,|\, \textsf{\&}T \,|\, \textsf{\&mut } T \,|\, {}$\\
    $\textsf{int}(N) \,|\, \{ x:\textsf{int} \mid P \}$
  }
  & \textbf{(Types)}
\end{array}
\]
