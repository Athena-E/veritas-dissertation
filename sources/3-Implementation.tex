...
\begin{center}
\begin{tabular}{l}
\texttt{synth\_expr(ctx, expr) -> Result<(typed\_expr, ty), TypeError>} \\
\texttt{check\_stmt(ctx, stmt) -> Result<(typed\_stmt, ctx'), TypeError>} \\
\texttt{check\_stmts(ctx, stmts) -> Result<(typed\_stmts, ctx'), TypeError>} \\
\texttt{check\_program(program) -> Result<typed\_program, TypeError>}
\end{tabular}
\end{center}
\end{tcolorbox}
