...
\]
The term $\iota$ denotes the expression's value in generated constraints. For a singleton it is the singleton index; for a refinement or existential type it is the symbolic term introduced during synthesis, not merely the binder name in the refinement. Boolean expressions also expose a guard proposition $\mathsf{guard}(e)$, for example $\mathsf{guard}(e_1 < e_2) = \iota_1 < \iota_2$. Conditional and loop rules use this guard rather than $\textsf{prop}(\textsf{bool})$, so branch facts such as \texttt{lo <= hi} enter the corresponding path context.
...
\subsection{Formal scope of the implemented rules}

The rules above and Appendix~\ref{app:source-rules} are a compact model of the implementation rather than a mechanised metatheory. Table~\ref{tab:formal-gap-closure} records the main places where the formal presentation has been tightened to match the scoped guarantee.
...
\begin{table}[H]
  ...
  \begin{tabular}{p{0.24\textwidth} p{0.34\textwidth} p{0.32\textwidth}}
    \hline
    \textbf{Issue} & \textbf{Rule treatment} & \textbf{Scope} \\
    \hline
    Branch facts & Conditions expose $\mathsf{guard}(e)$, used on then/else and loop paths. & Supports binary-search-style bounds reasoning. \\
    Symbolic values & Synthesis returns an SMT term $\iota$ for value-dependent obligations. & Avoids treating refinement binders as runtime values. \\
    Division and arithmetic & $\textsf{arith-safe}$ emits non-zero divisor and implemented signed-range obligations. & Only emitted side conditions are claimed. \\
    Array mutation & Stores require element compatibility and create a new logical array version. & Full heap/pointer memory safety is outside scope. \\
    Mutable array subtyping & Mutable arrays are invariant; covariant reasoning is read-only. & Prevents writes through widened mutable views. \\
    Physical DTAL & Spills, division expansion, branch state, and ownership events are explicit verifier state. & Encoder, runtime, and machine model remain trusted. \\
    \hline
  \end{tabular}
  \caption{Formal-rule clarifications supporting the scoped verifier guarantee.}
  \label{tab:formal-gap-closure}
\end{table}
