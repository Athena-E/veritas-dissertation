...
%
This chapter describes the implementation of the Veritas toolchain: a type-preserving compiler, an independent DTAL verifier, a compact runtime, and a bare-metal unikernel target. The chapter begins with a repository overview (Section~\ref{sec:repo-overview}). It then presents the formal definitions of the source language (Section~\ref{sec:source-language}) and DTAL target language~\ref{sec:dtal-target-lang}, then the compilation pipeline (Sections~\ref{sec:frontend}--\ref{sec:backend}), the verifier (Section~\ref{sec:verifier}), and the runtime and bare-metal components (Sections~\ref{sec:runtime}--\ref{sec:unikernel}). Finally, Section~\ref{sec:worked-example} traces a binary search program through the full pipeline as a worked example. 
%
...
\begin{itemize}
  \item \texttt{z3}\footnote{\url{https://docs.rs/z3/latest/z3/}} (v0.19, MIT): Rust bindings for the Z3 SMT solver~\cite{DeMoura2008Z3} (MIT-licensed), used for proof obligations in both the type checker and verifier. Z3 is a substantial external system; its correctness is assumed.
  \item \texttt{chumsky}\footnote{\url{https://docs.rs/chumsky/latest/chumsky/}} (v1.0.0-alpha.8, MIT): a parser combinator library used for the lexer and parser. It provides the parsing infrastructure, but all grammar rules are original.
  \item \texttt{im}\footnote{\url{https://docs.rs/im/latest/im/}} (v15.1, MPL-2.0+): persistent (structurally shared) data structures, used for efficient context cloning during type checking.
  \item \texttt{ariadne}\footnote{\url{https://docs.rs/ariadne/latest/ariadne/}} (v0.5, MIT): a diagnostic rendering library used for formatting error messages with source-location highlights. It handles the presentation layer only; all error detection and classification is original.
\end{itemize}
...
A separate mode, \texttt{--verify-dtal}, accepts a standalone \texttt{.dtal} file and verifies it without requiring the source, demonstrating that the verifier operates independently of the compiler frontend. The \texttt{--target\allowbreak-bare-metal} flag produces a Multiboot-compliant ELF for execution directly on QEMU\footnote{QEMU is an open-source machine emulator and virtualiser: \url{https://www.qemu.org/}.}.
...
The master type $\tau_m$ is set at declaration and never changes; the current type $\tau_c$ may be refined through control flow but must remain a subtype of $\tau_m$. 

% Mutable variables present a challenge for refinement types: an assignment \texttt{x = e} changes the value of \texttt{x}, potentially invalidating refinements that mention it. Veritas addresses this with \textit{master types}. 
To demonstrate: when a mutable variable is declared as \texttt{let mut x: T = e}, the type $T$ is recorded as the variable's master type $T_m$. The variable's \textit{current type} $T_c$ may be narrowed through assignments and control flow (e.g., after \texttt{if x > 0}, the current type narrows to $\{v\!:\!\mathtt{int} \mid v > 0\}$), but every assignment must satisfy $\tau_e <: T_m$. At control-flow joins, current types from each branch are merged using the join operation, but the result must remain a subtype of the master type. This prevents the invalidation of refinements if \texttt{x} is reassigned.
...
\begin{itemize}
    ...
    \item \textbf{Arithmetic:} \texttt{binop $r_d$, $r_{s1}$ (add, sub, mul, div, mod) | $r_{s2}$} | \texttt{addimm $r_d$, $r_s$, $n$} | \texttt{not $r_d$, $r_s$}
...
A subsequent conditional branch refines the constraint context on each outgoing edge. The fall-through path is captured by the verifier's local derivation rule:
$$
\frac{
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!60!black,title=\textsc{Core checker interfaces}]
\begin{center}
\begin{cminted}{rust}
...
\begin{figure}[H]
\centering
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!60!black,title=\textsc{TIR syntax}]
\begin{center}
\begin{cminted}{text}
...
The passes run in a fixed order: constant folding, peephole optimisation, copy propagation, dead code elimination, followed by LICM and load-fusion. This sequence is iterated to a fixed point, bounded to ten iterations in the current implementation.
