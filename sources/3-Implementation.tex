...
\noindent The third-party crates used in the implementation are:

\begin{itemize}
  \item \texttt{z3}\footnote{\url{https://docs.rs/z3/latest/z3/}} (v0.19, MIT): Rust bindings for the Z3 SMT solver~\cite{DeMoura2008Z3} (MIT-licensed), used for proof obligations in both the type checker and verifier.
  \item \texttt{chumsky}\footnote{\url{https://docs.rs/chumsky/latest/chumsky/}} (v1.0.0-alpha.8, MIT): Parser combinator library used for the lexer and parser; it provides the parsing infrastructure, but all grammar rules are original.
  ...
  \item \texttt{ariadne}\footnote{\url{https://docs.rs/ariadne/latest/ariadne/}} (v0.5, MIT): Diagnostic rendering library used for formatting error messages with source-location highlights; it handles the presentation layer only.
\end{itemize}
