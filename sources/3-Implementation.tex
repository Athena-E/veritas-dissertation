...
Figure~\ref{fig:pipeline} summarises the compilation pipeline. Each named representation is shown in sequence, with arrows indicating transformations. Safety evidence lowers from source refinements into verifier-facing DTAL type states and constraints; after DTAL verification, final x86-64 encoding no longer needs the annotations.
...
This section presents Veritas syntax and formal typing rules. They define the source-level contract the rest of the pipeline must preserve: any program accepted by the type checker satisfies the properties encoded in its types.
...
Veritas targets a first-order variant of Xi and Harper's DTAL design~\cite{Xi2001DTAL}. Xi and Harper formally established DTAL soundness, proving that well-typed programs cannot violate memory safety or type-system constraints.
...
This section describes V-DTAL's concrete instruction set and typing judgements, building on DTAL's theoretical foundation.
...
For performance, constraint provability uses two tiers. A fast syntactic check handles common cases (constraint already in context, simple entailment patterns) in $O(n)$ where $n$ is the context size. The SMT solver is invoked only when this is inconclusive. This matters because the verifier issues many more queries than the compiler: one per instruction rather than one per subtyping check.
