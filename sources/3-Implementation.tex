...
Figure~\ref{fig:pipeline} summarises the compilation pipeline. Each named representation is shown in sequence, and arrows indicate the transformations between them. Safety evidence is progressively translated from source refinements into verifier-facing DTAL type states and constraints; after DTAL verification, final encoding to x86-64 no longer needs the annotations.
...
This section presents the syntax and formal typing rules of the Veritas source language. These define the source-level contract that the rest of the pipeline must preserve: any program accepted by the type checker satisfies the properties encoded in its types.
...
Joins also preserve refinement information. Phi nodes for loop-carried or branch-merged values may carry existential constraints, for example recording that a loop induction variable lies in $[\mathit{start},\mathit{end})$ without fixing its concrete value. This gives later DTAL generation enough information to produce block-entry type states that the verifier can check.
...
Optimisation runs on virtual-register DTAL before physical allocation. The implemented pipeline performs constant folding, local peephole rewrites, copy propagation, dead-code elimination, loop-invariant code motion, and a Veritas-specific load-operation fusion pass. The important design point is that these transformations operate over typed assembly rather than an untyped IR: explicit constants, bounds facts, and verifier-facing effects are still present when optimisation runs. Rewritten instructions must retain verifier-facing type annotations, and instructions with semantic or proof relevance---stores, calls, branches, stack operations, ownership events, and constraint assertions---are treated as effects rather than removable bookkeeping.
...
The \textit{direct encoder} translates physically-allocated DTAL to x86-64 machine code. Because the physical allocation pass has already performed all x86-specific expansions (division, calling convention, prologues), the encoder is a deliberately narrow direct mapping: each DTAL instruction maps to exactly one x86-64 instruction (or a short fixed sequence for prologues and epilogues). The entire module is 309 lines in the current implementation.

This directness is deliberate and architecturally significant. Initially, I implemented a monolithic x86-64 lowering pass that performed register allocation, instruction selection, and encoding in a single trusted step. By splitting this into an untrusted physical allocation pass (verified by the DTAL verifier) and a compact encoder, most register-allocation and instruction-selection logic is moved outside the trusted path. The encoder remains trusted; its compactness provides an auditability argument for the final step from checked physical DTAL to machine code.
...
The verifier checks emitted DTAL from scratch using its own SMT queries; no proof state is passed from the source checker to the verifier. The translation of DTAL constraints into SMT queries follows the same proof-by-refutation approach as the compiler's oracle (Section~\ref{sec:smt-oracle}).
...
The compiler provides a compact set of runtime intrinsics that are linked into every executable. These are implemented as hand-assembled x86-64 machine code totalling approximately 210 bytes, prepended to the user's compiled code in the ELF output.
...
The \texttt{--target-bare-metal} flag in the Veritas CLI produces a Multiboot-compliant ELF32 that can boot directly on x86-64 hardware or QEMU\footnote{QEMU is an open-source machine emulator and virtualiser: \url{https://www.qemu.org/}.} without an operating system. This demonstrates that the same checked pipeline can produce OS-independent executable artifacts. For this target, the host operating-system kernel is removed from the normal execution path and replaced by a compact trusted bootstrap and runtime. The bootstrap, runtime intrinsics, hardware model, and QEMU behaviour remain outside the DTAL verifier's guarantee.
...
Aliasing is the next pressure point for a systems-language verifier: mutation through one name can invalidate facts assumed through another name, and unrestricted references make it unclear which part of the program owns a mutable object. Veritas addresses this by adding a Rust-inspired ownership and borrowing discipline: the source checker rejects obvious aliasing errors, while TIR and DTAL preserve ownership events explicitly so that the final low-level verifier can check that the backend has not introduced an illegal move, duplicated owner, or conflicting borrow.
