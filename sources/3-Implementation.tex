...
%
This chapter describes the implementation of the Veritas toolchain: a type-preserving compiler, an independent DTAL verifier, a compact runtime, and a bare-metal unikernel target. The chapter begins with the formal definitions of the source language (Section~\ref{sec:source-language}) and DTAL target language, then presents the compilation pipeline (Sections~\ref{sec:frontend}--\ref{sec:backend}), the verifier (Section~\ref{sec:verifier}), and the runtime and bare-metal components (Sections~\ref{sec:runtime}--\ref{sec:unikernel}). Section~\ref{sec:worked-example} traces a binary search program through the full pipeline as a worked example, and Section~\ref{sec:repo-overview} provides a repository overview.
%
...
\begin{gather*}
  \begin{aligned}
    c ::=\, & \underline{n} \,|\, \textsf{true} \,|\, \textsf{false}
  \end{aligned}
  \qquad \textbf{(Base values)}
\end{gather*}
